...
//
  ...
  final class WatchLibraryViewModel: ObservableObject {
    ...
    func loadAllSongs() {
      requestLibrary(type: "songs") { (result: Result<[Song], Error>) in
        switch result {
    ...
    func loadRadios() {
      requestLibrary(type: "radios") { (result: Result<[Radio], Error>) in
        switch result {
    ...
    func loadSongs(for album: Album) {
      ...
      requestLibrary(type: "albumSongs", parameters: ["id": album.id]) {
        (result: Result<[Song], Error>) in
        switch result {
        case .success(let songs):
    ...
    func loadSongs(for playlist: Playlist) {
      ...
      requestLibrary(type: "playlistSongs", parameters: ["id": playlist.id]) {
        (result: Result<[Song], Error>) in
        switch result {
        case .success(let songs):
    ...
    func loadAlbums(for artist: Artist) {
      ...
      requestLibrary(type: "artistAlbums", parameters: ["id": artist.id]) {
        (result: Result<[Album], Error>) in
        switch result {
        case .success(let albums):
